...
SIGMA_MIN = 0.05
REWARD_SCALE = 1.0
MODEL_PATH = "ddpg_humanoid_finalv6.pth"
BEST_MODEL_PATH = "ddpg_humanoid_best_new.pth"
...
for episode in range(EPISODES):
    ...
    for step in range(MAX_STEPS):
        ...
        # 💥 Improved reward shaping with lateral fix
        ...
        height_bonus = (torso_height - 0.8) * 5.0
        height_bonus = max(height_bonus, 0)

        forward_velocity_bonus = 1.5 * forward_bonus

        lateral_deviation = np.abs(next_state[1])  # deviation on y-axis
        lateral_penalty = 2.0 * lateral_deviation

        penalty = 1.0 * ctrl_cost

        reward = alive_bonus + forward_velocity_bonus + height_bonus - penalty - lateral_penalty
        reward *= REWARD_SCALE
